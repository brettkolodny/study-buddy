...
let make = (~setShowNewDeck) => {
  let (cards, setCards) = React.useState(() => list{})
  ...
  let updateDeckName = () => {
    open Webapi.Dom
    let currentDeckName = getElementById("deckName", document) |> getValue
  ...
  <div id="new-deck" className="open">
    ...
    <input type_="text" id="deckName" placeholder="Untitled" onBlur={_ => updateDeckName()} />
    <NewCard />
